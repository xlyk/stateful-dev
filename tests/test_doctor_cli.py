...
def test_doctor_reports_state_field_type_errors(tmp_path):
    state_path = tmp_path / "state.json"
    state_path.write_text(
        json.dumps(
            {
    ...
    payload = json.loads(result.stdout)
    assert payload["ok"] is False
    assert "version must be an integer" in payload["errors"]
    assert "attempts for plan:T1-one must be an integer" in payload["errors"]
